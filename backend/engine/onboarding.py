...
def copy_pack_to_customer(pack_root: str, customer_root: str,
    ...
    pack_root: packs/<pack_name>/ 的绝对路径
    customer_root: customers/<customer_id>/ 的绝对路径
    生成：customers/<id>/ontology/（TTL + Action）+ config.yaml + data/
    ...
    # copy domains/
    domains_src = os.path.join(pack_root, "domains")
    ...
    # copy processes/
    processes_src = os.path.join(pack_root, "processes")
    if os.path.isdir(processes_src):
        shutil.copytree(processes_src, os.path.join(ontology_dst, "processes"),
                        dirs_exist_ok=True)

    # 生成 config.yaml
    enabled_domains = _list_subdirs(os.path.join(ontology_dst, "domains"))
    enabled_processes = _list_subdirs(os.path.join(ontology_dst, "processes"))
    config = {
