...
> **状态**：✅ 当前。
> **配套**：模板见 [`templates/`](./templates/)（填法见 [`02-templates.md`](./02-templates.md)）；完整例子见 [`03-worked-example-customerA.md`](./03-worked-example-customerA.md)。
...
**做什么**：填 [`templates/ws-tools.py.template`](./templates/ws-tools.py.template) → 产出 `tools.py`。
**放在**：`workspace/<pack>/skills/<process>/tools.py`。

导出 `TOOLS = [...]` 列表（`@tool` 函数）。被 `_build_ws_tools(ws_name)` 自动聚合。工具经 `agent.tools.shared` 的 helper 装配（`_get_repo`/`_get_executor`）。
...
**重启后端**：`bootstrap()` 自动发现 `workspace/<pack>/workspace.py`，注册进 pack 注册表。`_build_ws_tools(ws_name)` 聚合工具，`_build_ws_prompt(ws_name)` 合并本体提示，`_build_ws_skills(ws_name)` 收录 Skill。**零改内核，重启即生效。**
