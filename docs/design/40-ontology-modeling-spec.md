...
每个 Object Type **必须** 声明以下元数据（retail 行业包的 TTL 已补齐 `status`/`edits_only_via_actions`；`visibility` 可选，见架构文档 §3.1）：
...
- 状态机迁移作为副作用的一种：`{ type: state_transition, target: Task, from: pending_approval, to: approved }`，合法转换表（dict）见架构文档 §2.4。
