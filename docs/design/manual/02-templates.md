...
| 模板 | 产出 | 用于 Phase |
|------|------|-----------|
| [`pack.py.template`](./templates/pack.py.template) | `workspace/<pack>/pack.py` | F（注册） |
...
### `pack.py`：IndustryPack + CapabilityDomain + ValueChainProcess
...
| 模板 | 常见错误 | 正确做法 |
|------|---------|---------|
| `pack.py` | `register_pack` 忘了调 / 路径用相对路径 | 必须 `register_pack(PACK)`；路径用 `os.path.join(_BASE, ...)` 绝对路径 |
...
| `seed-object.json` | 忘 `workspace_name` | 每条带 `workspace_name: "customer_default"` |
...
模板里的占位符用 `<...>` 或 `{{...}}` 标记。替换时：
- `<pack>` → 行业包名（snake_case），如 `equipment_repair`
