# Worked Example：门店设备维修工单（第二个行业包端到端）

> **状态**：✅ 当前。本文对照真实代码 `workspace/equipment_repair/` 写成（非臆造）。
> **目的**：证明多行业包并存 + 零改内核 + 无折扣概念也能跑。可作为新行业包接入的完整对照。
...
├── pack.py                                     # IndustryPack 声明（register_pack）
├── ontology/
│   └── domains/
...
## 3. pack.py 实际结构（对照 `workspace/equipment_repair/pack.py`）
...
`workspace/equipment_repair/skills/repair_workflow/state_machine.py`：
```
...
启动后端，`bootstrap()` 发现 `workspace/equipment_repair/pack.py` → 注册进 pack 注册表。

### 5.1 多行业包并存的工具与 Skill 聚合

- `main._aggregate_pack_tools()` 聚合 retail + equipment_repair 的专属工具 → `query_near_expiry` + `query_repair_tickets`
- `_aggregate_skill_paths()` 收录两行业包的 Skill
- `_build_combined_prompt()` 合并两行业包的本体知识

### 5.2 可验证的操作

```bash
# 查设备
query_entity(entity_type="Equipment", workspace_name="customer_default")
...
新增行业包是**纯增量**：建 `workspace/<pack>/`（pack.py + 能力域本体 + 可选 state_machine/tools/skills）+ `data/` 种子。重启即生效，零改内核。详见 [`01-onboarding.md`](./01-onboarding.md)。
