"""测试 Repository 双字段过滤（架构 §3.3：workspace_name 硬隔离 + org_unit_id 范围）。"""
import json
import os
...
def test_read_filters_by_customer(tmp_path):
    """不同 customer 数据硬隔离。"""
    _seed(str(tmp_path), [
        {"id": "s1", "workspace_name": "c1", "org_unit_id": "store_001", "name": "A"},
        {"id": "s2", "workspace_name": "c2", "org_unit_id": "store_001", "name": "B"},
    ])
    repo = JSONFileRepository(data_dir=str(tmp_path), registry=_registry())
    tc1 = TenantContext(workspace_name="c1", org_unit_id="*")
    tc2 = TenantContext(workspace_name="c2", org_unit_id="*")
...
def test_read_filters_by_org_unit(tmp_path):
    ...
    _seed(str(tmp_path), [
        {"id": "s1", "workspace_name": "c1", "org_unit_id": "store_001", "name": "A"},
        {"id": "s2", "workspace_name": "c1", "org_unit_id": "store_002", "name": "B"},
    ])
    repo = JSONFileRepository(data_dir=str(tmp_path), registry=_registry())
...
def test_write_stamps_customer_and_org(tmp_path):
    """写入时盖上 workspace_name + org_unit_id。"""
    _seed(str(tmp_path), [])
    repo = JSONFileRepository(data_dir=str(tmp_path), registry=_registry())
    tc = TenantContext(workspace_name="c1", org_unit_id="store_001")
    repo.write("Store", tc, {"id": "s_new", "name": "新"}, create=True)
    rows = repo.read("Store", tc)
    assert rows[0]["workspace_name"] == "c1"
