...
| `workspace/<pack>/skills/<process>/` | **工作目录** | 场景单元（SKILL.md + tools.py + 可选 automation.py/state_machine.py） |
...
```
┌─────────────────────────────────────────────────────────────┐
│ Step 1（每次新增工作目录）：按 01-onboarding.md 走 Phase A-F    │
│   A 建模（能力域 TTL）→ B Action 契约（YAML）→ C 状态机      │
│   → D 种子数据 → E 工具/Skill → F 注册（workspace.py，重启即生效） │
├─────────────────────────────────────────────────────────────┤
│ Step 2：验证清单（对照建模规范 §9）                            │
│   命名 / 元数据 / via 归属 / Action 边界 / 原子性 / 一致性     │
├─────────────────────────────────────────────────────────────┤
│ Step 3：重启后端，bootstrap 自动发现新工作目录                   │
└─────────────────────────────────────────────────────────────┘
```
...
- 工作目录名：`snake_case`，如 `retail` / `customerA` / `inventory_count`。
- 目录：`workspace/<pack_name>/`，内含 `workspace.py`（注册入口）。
...
- prefix 只需在该能力域的 TTL 内一致；不影响其它域/工作目录。
...
- Action api_name：`snake_case` 动词短语，**建议加流程前缀**避免跨工作目录冲突（如 `create_clearance_task` 而非 `create_task`）。
...
**应当建工作目录**：一组业务实体有独立的生命周期、受治理的写操作（Action）、跨多步的工作流。如：设备维修工单、库存盘点、会员退换货。

**不应当建工作目录**（用现有工具即可）：一次性查询、纯读取报表、单条 CRUD。这些用内核 `query_entity` / `create_entity`（降级 CRUD）即可，不必建模成工作目录。

判断见建模规范 §3.4（何时建新 Object Type）与 §5.2（何时建 Action Type）—— 工作目录是"一组能力域 + 价值链流程"的集合，粒度比单个 Object 大。
