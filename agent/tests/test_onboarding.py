...
def test_copy_pack_creates_customer_ontology(tmp_path, monkeypatch):
    """copy 行业包到客户目录：生成 ontology/ + config.yaml（workspace 重构版）。"""
    # mock workspace/retail 路径指向临时 pack
    pack_dir = tmp_path / "workspace" / "retail"
    (pack_dir / "ontology" / "domains" / "marketing").mkdir(parents=True)
    (pack_dir / "ontology" / "domains" / "marketing" / "domain.ttl").write_text(
        '@prefix store: <http://x#> .\n@store:Product a rdfs:Class ;\n'
        '    store:properties "id:string" ; store:storage "products.json" .', encoding="utf-8")
    (pack_dir / "ontology" / "domains" / "marketing" / "actions").mkdir()
    (pack_dir / "ontology" / "domains" / "marketing" / "actions" / "test.yaml").write_text(
        "api_name: test\ntarget_object_type: Product\nedits_object_types: []\n"
        "parameters: []\nside_effects: []\n", encoding="utf-8")
    # pack 的 data/（workspace 重构：data 在 pack 根下，随 copy 带入客户）
    (pack_dir / "data").mkdir()
