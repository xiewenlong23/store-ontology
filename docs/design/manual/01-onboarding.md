...
1. 这组业务实体有**独立生命周期 + 受治理写操作 + 多步工作流**吗？是 → 适合建行业包。否 → 用内核 CRUD 工具即可（见 [`00-overview.md`](./00-overview.md) §4）。
...
步骤：
1. 识别能力域。把实体按业务职能分组：营销相关 → `marketing`，组织相关 → `organization`，财务相关 → `finance`。一个行业包可有多个能力域。
...
**放在**：`workspace/<pack>/skills/<process>/state_machine.py`（如 `workspace/equipment_repair/skills/repair_workflow/state_machine.py`）。

步骤：
1. 定义 `<WORKFLOW_OBJECT>_TRANSITIONS` dict（键=当前状态，值=允许的下一状态列表）+ `TERMINAL_STATES` 集合。
2. `pack.py` 会 import 这两个并填入 `ValueChainProcess.state_transitions` / `terminal_states`（见 Phase F）。
...
**无工作流的行业包跳过本步**：不建 ValueChainProcess，只建 CapabilityDomain。
...
2. 每条记录带 `workspace_name`（默认 `customer_default`）；需 org 隔离的带 `org_unit_id`。
...
## Phase F：注册与接线（建 pack.py）

**做什么**：填 [`templates/pack.py.template`](./templates/pack.py.template) → 产出 `pack.py`。
**放在**：`workspace/<pack>/pack.py`。
...
**重启后端**：`bootstrap()` 自动发现 `workspace/<pack>/pack.py`，注册进 pack 注册表。`main._aggregate_pack_tools()` 聚合工具，`_build_combined_prompt()` 合并本体提示，`_aggregate_skill_paths()` 收录 Skill。**零改内核，重启即生效。**
