...
> **适用范围**：OntologyAgent 平台内核本体 + 所有 vertical（零售、物流、制造……）的领域建模
> **关联文档**：`docs/superpowers/specs/2026-06-20-ontologyagent-target-architecture-design.md`（目标架构，下称"架构 spec"）、`backend/ontology/store.ttl`（现有零售 vertical 本体）
> **性质**：本文件是**规范性**文档（规范 = 必须遵守的约定）。它不解释"系统是什么"（那是架构 spec 的职责），只规定"怎么建模才合规"。
...
- 所有示例取自现有零售临期 vertical（`store.ttl`），既是样板也是回归基线。
...
**本体的定位是组织的运营层（operational layer）/ 数字孪生**——它位于原始数据之上、应用之下，把数字资产连接到现实世界实体（门店、商品、任务）。它不是数据库 schema 的镜像，而是用业务语言描述"这个组织在运营什么、能做什么"。这与 Palantir Foundry Ontology 的定位一致（详见 `docs/palantir-ontology-docs/summary.md` §一）。
...
**不承载**计算逻辑（不引入 Palantir 式 Function 作为本体元素），计算逻辑是普通 Python 模块，通过 Tool 暴露给 LLM。详细分工见架构 spec §1。Interface Type（抽象形状）与 Shared Property（跨类型共享属性）列为 v2，MVP 不实现（见 §1.3）。
...
7. **edits-only-via-actions**：核心业务实体可锁定为"只能通过 Action 修改"，防止通用 CRUD 绕过治理（架构 spec §1.3）。
...
|---|---|---|
| **Function** 作为本体元素（注册到本体、被 Workshop/派生列/聚合发现绑定） | **不引入** Function 本体元素。计算逻辑是普通 Python 模块，经 Tool 暴露给 LLM | agent 时代消费者是 LLM，计算经 Tool（schema 注入 prompt）调用即足够；Tool 就是 LLM 时代的 Function（架构 spec §1.1） |
...
| TTL 前缀 | vertical 级常量（现 `store:`） | `store:Store`、`store:clearance` |

### 2.2 硬约束

- **必须** 同时提供中英文 `rdfs:label`：`"门店"@zh , "Store"@en`。中文供 LLM/UI 理解，英文供代码与跨 vertical 复用。
...
每个 Object Type **必须** 声明以下元数据（当前 `store.ttl` 缺 `status`/`visibility`/`edits_only_via_actions`，需在内核元数据扩展时补齐，见架构 spec §3.1）：
...
- rid（Palantir 式持久化标识）是 v2 概念，MVP 用 `id` 即可（架构 spec 附录 B.5）。

### 3.3 属性规范

- 每个属性声明 `name:type`，类型取自 §6.1 白名单。
- `description` 里写明业务含义；易混字段写明单位（§6.2）。
- 枚举属性类型写枚举类型名，如 `status:ProductStatus`、`discount_tier:DiscountTier`。枚举类型 **必须** 在代码侧（`models/schemas.py`）有对应的 `Enum` 定义，且值与本体一致。
...
`store.ttl` 现有 bug：`manages: Employee → Store, via="manager_id"`，但 `manager_id` 是 **Store** 的属性（不是 Employee 的）。`traverse_relation("Employee", "emp_001", "manages")` 能跑通纯属巧合（碰巧 `store_001.manager_id == "emp_001"`）。正确写法：
...
Action Type = 受治理业务事务的 **声明式变更契约**。它是 LLM 与后端自动化共用的变更入口（架构 spec §1.2、§1.4）。

### 5.1 必备要素

存储格式为 **YAML**（`ontology/actions/{api_name}.yaml`），不用 TTL（架构 spec §3.4）：
...
> 核心业务实体（NearExpiryProduct、Task 等）应标记 `edits_only_via_actions: true`，使上述"不应当"场景也无法绕过 Action（架构 spec §1.3）。通用 CRUD（`create_entity`/`update_entity`）**禁止** 用于这些实体。
...
- 状态机迁移作为副作用的一种：`{ type: state_transition, target: Task, from: pending_approval, to: approved }`，合法转换表（dict）见架构 spec §1.5。

### 5.6 原子性与可组合性（借鉴 Palantir）

Action 是 Ontology 中**改变数据的交易单位**，借鉴 Palantir 的两条关键性质：

- **原子性**：一个 Action 是一笔事务，要么全部成功，要么全部回滚。`confirm_action` 执行多个 Object 的多处修改时，必须用原子写入（临时文件 + rename）保证不出现"改了一半"的中间态（架构 spec §C.2）。失败时不留部分写入。
...
- 路径：`data/tenant/{tenant_id}/{storage_file}`。租户隔离由路径承载（架构 spec §3.3）。
...
- Object/Link Type 的权威定义在 **TTL**（`store.ttl`），代码常量（如 `models/schemas.py` 的 `LinkTypes`）**必须** 与 TTL 一致。
...
| 6 | **通用 CRUD 绕过 Action 治理** | `update_entity` 能直接改 NearExpiryProduct.status，绕过审批 | edits-only-via-actions（§5.2、架构 spec §1.3） |
| 7 | **状态字段无状态机约束** | Task.status 自由改写，无合法迁移约束 | 状态迁移用 Action + 迁移表（架构 spec §1.5） |
...
以现有零售 vertical 为例，展示 Object / Link / Action 三者的完整建模。这是新 vertical 建模的参考样板。
...
借鉴 Palantir summary.md §十的 6 步顺序，本地化为"无 Function"的 5 步。新建一个 vertical 或扩展领域时，按此顺序建模：
...
本规范是架构 spec 中"怎么建模"部分的展开与硬化，同时是 Palantir 方法论在 LLM agent 时代的落地裁剪。架构变更或 Palantir 借鉴策略调整时，三份文档（架构 spec、本规范、Palantir summary）需同步。
