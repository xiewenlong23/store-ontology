...
多行业包改造后：状态迁移表来自 ValueChainProcess（per-process），不再全局硬编码。
clearance 的迁移表作为默认保留（向后兼容 + 作 ValueChainProcess.state_transitions 的范例）。
"""

# clearance 价值链流程的状态迁移表（向后兼容 + ValueChainProcess 范例）
TASK_TRANSITIONS = {
...
def is_valid_transition(from_status: str, to_status: str,
    ...
    多价值链流程场景从 ValueChainProcess 传入该流程自己的表。
    """
